...
- `BRAIN_AUTH_ENABLED` controls auth; related settings include access token lifetime, refresh token lifetime, password file, users file, state path, scopes, PKCE, `BRAIN_AUTH_TOKEN`, and `BRAIN_AUTH_SUPERUSER_IDS` in prod, staging, and QA.
- `BRAIN_COGNEE_*_DATASET` settings separate memory, data, sources, palate, and chat continuity. `BRAIN_COGNEE_ENABLED`, `BRAIN_COGNEE_RECALL_ENABLED`, `BRAIN_COGNEE_RECALL_TOP_K`, `BRAIN_COGNEE_SYNC_ON_INGEST`, and `BRAIN_COGNEE_SYNC_ON_INGEST_SWEEP_LIMIT` control Cognee availability, recall, and ingest synchronization.
...
<!-- brain-doc-source-hash: b080b7410209c2a99397960770b5a0431c0e958747222a982d8d7728797719b5 -->
