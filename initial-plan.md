...
This plan intentionally skips the direct Graphiti lane. Cognee-native temporal memory is tested first via `remember(..., temporal_cognify=True)` and `recall(..., search_type="TEMPORAL")`.
...
When `PROFILE=local`, the system must fail fast if configuration would call OpenAI, Gemini, or another cloud provider.
